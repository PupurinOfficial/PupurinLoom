...
define config.name = _("a new Pupurin Loom game")


## Pupurin° Loom：启用开发者模式以支持图形编辑器「从这里开始玩」（Ren'Py --warp）。
## 发布正式版时如需关闭，删除此行或将 True 改为 False 即可。
...
define build.name = "anewPupurinLoomgame"
...
define config.save_directory = "anewPupurinLoomgame-1785742498"
...
init python:
    ...
    ## 排除打包输出目录：否则旧的打包产物会被重新打进新包，导致越打越大、越来越慢。
